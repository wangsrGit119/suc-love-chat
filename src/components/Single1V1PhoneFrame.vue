<template>
    <div class="Single1V1Phone">
        <audio id="localAudio" autoplay hidden muted> </audio>
        <audio id="remoteAudio" autoplay  controls v-show="!sharing"> </audio>
        <video id="remoteVideo" controls="controls" v-show="sharing"> </video>
        <div v-if="isCreateOfferOwner">
            <el-button type="primary" @click="shareScreenStream" round v-if="isLocalStream">分享屏幕</el-button>
            <el-button type="warning" @click="removeScreenStream" round v-if="sharing">关闭分享</el-button>
        </div>
    </div>
</template>
...
<script>
    ...
    methods:{
        ...
        nativeMedia(){
            const that = this;
            console.log("open local Audio",that.localStream)
            let audio = document.querySelector('#localAudio');
            // 旧的浏览器可能没有srcObject
            if ("srcObject" in audio) {
                audio.srcObject = that.localStream;
            } else {
                audio.src = window.URL.createObjectURL(that.localStream);
            }
        ...
        //关闭本地画面
        closeNativeVideo(){
            let video = document.querySelector('#localAudio');
            video.srcObject = null;
        },
        //挂断
        hangUp(){
        ...
        initPeer(){
            ...
            that.pc.onaddstream = (event) => {
                ...
                let audio = document.querySelector('#remoteAudio');
...
<style scoped>
    ...
    .Single1V1Phone video{
        object-fit:fill;
        width: 320px;
        height: 200px;
    }
    .Single1V1Phone audio{
        object-fit:fill;
        width: 320px;
        height: 200px;
    }
